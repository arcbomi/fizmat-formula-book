#https://github.com/xhuihuix/docsify-auto-sidebar-gen/tree/master
#do not run it 
...
def save_structure(root_dir, base_dir=base_dir, depth=0):
    ...
    if depth != 0:
        ...
        if create_depth == 0:
            subdir_structure += "- " + "Home Page" + '\n'
        else:
            subdir_structure += "- [" + "Home Page" + "](" + relpath(root, base_dir) + '\)\n'
    ...
    if depth == 1:
        subdir_structure = "- [" + "Return to Home" + "](" + "" + '\?id=main)\n' + subdir_structure
    elif depth != 0:
        abs_pre_path = abspath(join(root, ".."))
        rel_pre_path = relpath(abs_pre_path, base_dir)
        subdir_structure = "- [" + "Back to Previous page" + "](" + rel_pre_path + '\)\n' + subdir_structure
